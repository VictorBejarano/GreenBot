...
export class QualityCardComponent implements OnDestroy, OnInit {
  private alive = true;
  ...
  ngOnInit() {
    ...
    let dataStorage = localStorage.getItem("air_quality_data");
  ...
  startReceivingLiveData() {
    ...
    this.intervalSubscription = this.socketService
      ...
      .subscribe((res) => {
        this.earningLiveUpdateCardData = {
          ...this.earningLiveUpdateCardData,
          dailyIncome: res.data.air_quality,
        };
        let temporal = [...this.liveUpdateChartData];
        temporal.push({
          value: [res.data.timestamp, res.data.air_quality],
        });
        temporal.shift();
        this.liveUpdateChartData = temporal;
        localStorage.setItem("air_quality_data", JSON.stringify(temporal));
        console.log(res);
      });
  }
